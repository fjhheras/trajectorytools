...
def get_bouts(tr, smooth_sigma, order):
    ...
    for focal in range(tr.number_of_individuals):
        ...
        starting_bouts = signal.argrelextrema(tr_s.speed[:, focal],
                                              np.less, order=order)[0]
        bout_peaks = signal.argrelextrema(tr_s.speed[:, focal],
                                          np.greater, order=order)[0]
        print(len(starting_bouts), len(bout_peaks))
        bouts = []
        bout = []
        for frame in range(tr.number_of_frames):
            if not np.isnan(tr.s[frame, focal, 0]):
                if frame in starting_bouts:
                    if len(bout) == 0:
                        bout.append(frame)
                    elif len(bout) == 1:
                        bout=[]
                        bout.append(frame)
                elif frame in bout_peaks:
                    if len(bout) == 1:
                        bout.append(frame)
                        bouts.append(bout)
                        bout = []
        ...
        starting_bouts, bout_peaks = np.asarray(starting_bouts), np.asarray(bout_peaks)
...
if __name__ == '__main__':
    ...
    all_starting_bouts, all_bout_peaks, tr_s = get_bouts(tr,
                                                         smooth_sigma=2,
                                                         order=2)

    def plot_slide_time(ax, t, focal):
        time_range = (t, t + 200)  # in frames
        ...
        ax.plot(np.asarray(frame_range), tr_s.speed[frame_range, focal], c='k')
